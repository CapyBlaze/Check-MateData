...
export function Captcha({ 
    ...
    const verifyCaptcha = () => {
        if (gameChess.fen().split(' ')[0] === gameModel.fen().split(' ')[0]) {
            onVerify?.(true);
            onNotification?.('Captcha solved successfully!', 'success');

        } else {
            onNotification?.('Captcha verification failed. Please try again.', 'error');
        }
    }
    

    return (
        <div className="overflow-hidden rounded-md" draggable="false">
